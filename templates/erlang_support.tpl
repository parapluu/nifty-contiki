...
'{{name}}'(Handler, Mote{% with arguments=symbols|fetch:name %}{% for argument in arguments %}{% if argument|is_argument %},A{{argument|getNth:2}}{% endif %}{% endfor %}{% endwith %}) ->
   ok = '{{name}}_call'(Handler, Mote{% with arguments=symbols|fetch:name %}{% for argument in arguments %}{% if argument|is_argument %},A{{argument|getNth:2}}{% endif %}{% endfor %}{% endwith %}),
   '{{name}}_result'(Handler, Mote).

'{{name}}_call'(Handler, Mote{% with arguments=symbols|fetch:name %}{% for argument in arguments %}{% if argument|is_argument %},A{{argument|getNth:2}}{% endif %}{% endfor %}{% endwith %}) ->
   F = [
